...
func _create_hud_frame():
	...
	hud_frame.position = Vector2(10, viewport_height - HUD_FRAME_SIZE.y - 10)
...
func create_health_bar():
	...
	var health_container = Control.new()
	health_container.name = "HealthContainer"
	health_container.position = Vector2(viewport_size.x - HEALTH_BAR_WIDTH - 30, 20)

	var bg = ColorRect.new()
	bg.name = "HealthBG"
	bg.color = Color(0.2, 0.2, 0.2, 0.8)
	bg.size = Vector2(HEALTH_BAR_WIDTH, 12)
